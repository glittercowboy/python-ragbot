...
class ClaudeService:
    ...
    # Modified version of generate_response


async def generate_response(self, user_query: str,
                            context_entries: List[Dict[Any, Any]]) -> str:
    """
    ...
    try:
        ...
        for entry in context_entries:
            ...
            if categories:
                formatted_entry = f"[Categories: {
                    ', '.join(categories)}] {entry_text}"
            else:
        ...
        # Generate response - remove the await since Anthropic's client is
        # synchronous
        ...
        logger.info(
            f"Generated Claude response for query: {user_query[:50]}...")
        return response_text

    except Exception as e:
        logger.error(f"Error generating Claude response: {e}")
        return "I'm sorry, I encountered an error while processing your question. Please try again."
    # Open claude_service.py and modify the generate_game_question and
    # generate_response methods:

# Modified version of generate_game_question
...
async def generate_game_question(self) -> str:
    ...
    try:
        ...
        # Create a synchronous request - Anthropic's Python client doesn't
        # support async natively
        response = self.client.messages.create(
            model=self.model,
            max_tokens=200,
            system=system_prompt,
